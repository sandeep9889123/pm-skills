...
`cases.json` includes adversarial scenarios for:
...
These cases directly exercise the failure modes encoded in `reliability/scenario_matrix.json`.
...
```json
{
  "case_id": "ET1_SINGLE_CLIENT_DEMAND",
  "evaluator": "independent-reviewer",
  "dimensions": {
    "evidence_integrity": {"score": 5, "rationale": "Separates observed demand from inference."},
    "demand_independence": {"score": 4, "rationale": "Requests more independent signals but misses one source-concentration caveat."}
  }
}
```
...
python evaluation/score_output.py \
  --case ET1_SINGLE_CLIENT_DEMAND \
  --output evaluation/runs/claude/ET1.md \
  --judgement evaluation/runs/claude/ET1.judgement.json
```
...
The key metric for the original competitor problem is **first-run success**, not whether the model can recover after the user tells it that it missed something.
